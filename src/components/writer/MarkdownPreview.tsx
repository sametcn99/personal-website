...
export function MarkdownPreview({ content }: MarkdownPreviewProps) {
    ...
    const parseMarkdown = (text: string) => {
        ...
        while (currentIndex < lines.length) {
            ...
            // Unordered List
                ...
                elements.push(
                    <UnorderedList key={key}>
                        {listItems.map((item) => (
                            <ListItemComponent key={`${key}-item-${item.slice(0, 10)}-${Math.random()}`}>{parseInlineMarkdown(item)}</ListItemComponent>
                        ))}
                    </UnorderedList>
                )
            ...
            // Regular paragraph
                ...
                elements.push(<Paragraph key={key}>{parseInlineMarkdown(line)}</Paragraph>)
            }
    ...
    const parseInlineMarkdown = (text: string): React.ReactNode => {
        ...
        return parts.map((part, index) => {
            ...
            return part.split(/(\*\*[^*]+\*\*|\*[^*]+\*|`[^`]+`)/g).map((segment, segIndex) => {
                ...
                if (segment.startsWith('**') && segment.endsWith('**')) {
                    ...
                    return <CodeComponent key={segKey}>{segment.slice(1, -1)}</CodeComponent>
                }
                return segment
            })
        }).filter(Boolean)
